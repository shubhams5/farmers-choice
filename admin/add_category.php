...
if(isset($_POST['submit_color']))
{
  ...
  $targetdir = "uploads/";
    ...
    $filetype = pathinfo($targetFiledir, PATHINFO_EXTENSION);
        ...
        $sql = "INSERT into `category` (category, icon, side_ban) values ('$color', '$filename','$filename1' )";
...
if(isset($_POST['submit_size']))
{
    ...
    extract($_POST);
        ...
        $sql = "INSERT into `subcategory` (subcategory, icon, parent) values ('$subcat', '$filename', '$mcategory')";
        $query = mysqli_query($connect, $sql) or die(mysqli_error($connect));
        if($query)
        {
              header("Location: add_category.php");
